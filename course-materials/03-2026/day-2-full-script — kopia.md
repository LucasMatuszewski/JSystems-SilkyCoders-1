# Dzien 2 — Pelny scenariusz prowadzenia
**Temat: Od pomyslu do projektu — AI jako Twoj PM, Architekt i Konfigurator**
...
I jeszcze jedno: settings.json w projekcie nadpisuje globalny. Czyli mozecie miec liberalne ustawienia globalne do eksperymentow, i restrykcyjne na poziomie projektu produkcyjnego. Separacja 100%."
...
Przypomnijcie sobie projekt: chatbot Sinsay do reklamacji i zwrotow.
Bede prosil o aktywna prace — bedziesz odpowiadal na pytania agenta
i krytykowal wygenerowane dokumenty.
```

---

## 11:10–12:15 — Modul 2.1: AI jako Twoj PM — PRD, User Stories, Requirements
⏱️ 65 min
...
Najpopularniejsze MCP ktore sa uzyteczne bezposrednio dla Was:"
...
4. Test CLAUDE.md — otworzcie NOWA sesje claude i wklejcie:
   "Przeczytaj CLAUDE.md i odpowiedz:
    ...
    3) Jakie sa kluczowe zasady pracy?
...
2) pom.xml z zaleznosciami zgodnymi z ADR:
...
Kazdy w bloku @startuml ... @enduml
Dla Sequence Diagram dodaj @note przy:
- miejscach gdzie moze wystapic blad sieci
- miejscach z timeoutem
- miejscach z danymi wrazliwymi RODO
```
...
## Agent Instructions (kiedy pytac o zgode, jak zglaszac problemy)
## Available Skills (/commit, /review, /prd-sync)
## Sub-agents (Backend, Frontend, Documents, QA)

Max 200 linii. Konkretne zasady, nie ogolniki.
Zapisz jako CLAUDE.md w root projektu.
```
...
2) pom.xml z zaleznosciami zgodnymi z ADR-001
...
Przeczytaj CLAUDE.md i odpowiedz:
1) Czym jest ten projekt i jaki problem rozwiazuje?
2) Jaki stack technologiczny i dlaczego ten wybor?
3) Jakie sa kluczowe zasady pracy (top 5)?
4) Co mozesz robic samodzielnie, a o co musisz pytac?
5) Jakie skills i sub-agenci sa dostepni?
```
...
| Problem | Rozwiazanie |
|---|---|
| `settings.json` nie jest respektowany | Sprawdz lokalizacje: `.claude/settings.json` w root projektu. Jesli globalny — `~/.claude/settings.json` |
| Agent pyta o zgode na rzeczy w `allow` | Sprawdz skladnie: `"Bash(mvn:*)"` — musi byc dokladna komenda przed `:*`. Uzyj `/doctor` |
| ACP nie widac w IntelliJ | Sprawdz wersje: Help → About. Potrzeba 2025.3+. Zaktualizuj lub uzyj CLI |
| Context7 MCP nie dziala | Sprawdz w CC: `/mcp`. Zrestartuj claude po zmianie settings.json. `npm install -g @upstash/context7-mcp` |
| IntelliJ MCP nie przekazuje diagnostyki | Wlacz opcje "Pass IntelliJ MCP server" przy konfiguracji ACP. Sprawdz czy projekt jest otwarty w IntelliJ |
| Ctrl+G nic nie robi | Sprawdz: `echo $EDITOR`. Musi byc ustawiony i dostepny w PATH. Ustaw: `export EDITOR="code --wait"` |
| CLAUDE.md ignorowany przez agenta | Plik musi byc w root katalogu projektu (gdzie uruchamiasz `claude`). Sprawdz przez `/memory` |
| `mvn compile` nie dziala | `java --version` — potrzeba Java 21. `echo $JAVA_HOME`. Jesli brak: zainstaluj przez SDK Manager lub sciezke |
...
## APPENDIX D — Plan B: jesli mocno za malo czasu

Jesli masz 30+ minut opoznienia:

| Co przycinamy | Jak |
|---|---|
| Sandbox cwiczenie | Skroc do 3 min: tylko pokaz settings.json, bez cwiczenia |
